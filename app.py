...
# '@' ~에서 라는 뜻을 가지고 있는 atSign은 
# '데코레이터'라는 문법으로 객체지향 언어에서 매우 자주 기용하며, 
# node.js의 Nest.js도 데코레이터를 사용한다. 
# flask 에서는  아래와 같은 방식으로 라우팅을 설정한다.
@app.route('/')
# 함수 정의가 아래가 끝이다. 중괄호를 사용하지 않기 때문에 들여쓰기가 매우 중요하다.
def index():
    return render_template('index.html')
# 위의 함수는 라우팅을 통해 '/'로 들어오는 요청을 처리하는 함수로, 
# 직관적으로 render를 위한 것이라는 것을 쉽게 알아챌 수 있다.

# 아래의 조건문은
# __ 언더스코어가 두개씩 들어간 코드는 일반적으로 
# 파이썬 언어자체의 특수한 문법을 나타내는 경우가 많다.
# 파이썬 코어 
